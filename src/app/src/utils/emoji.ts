...
export const emojiList: Record<string, EmojiItem> = {
  '🇦': {
...
export const emojiNameToUnicodeMap = new Map<string, string>()
for (const [key, value] of Object.entries(emojiList)) {
  emojiNameToUnicodeMap.set(value.name, key)
  for (const emoji of value.shortcodes) {
    emojiNameToUnicodeMap.set(emoji, key)
  }
}

export function getEmojiUnicode(name: string) {
  return emojiNameToUnicodeMap.get(name) || name
}

export function getEmojiName(unicode: string) {
  return emojiList[unicode]?.name || unicode
}

export function remarkEmojiPlugin() {
  return function (tree: Nodes) {
    findAndReplace(tree, [EMOJI_REGEXP, (match: string) => {
      ...
      if (emojiUnicode) {
        return {
          ...
          data: {
            hName: 'span',
            hProperties: { role: 'img', ariaLabel: match },
            hChildren: [{ type: 'text', value: emojiUnicode }],
          },
        }
      }
      return match
