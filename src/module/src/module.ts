...
export interface ModuleOptions {
  ...
  auth?: {
     ...
     * Use Supabase as an authentication provider.
     */
    supabase?: {
      /**
      ...
      key?: string
      /**
       * Login URL to redirect to if no session is found.
       * @default '/login'
       */
      loginUrl?: string
    }
...
export default defineNuxtModule<ModuleOptions>({
  ...
  defaults: {
    ...
    auth: {
      ...
      supabase: {
        ...
        loginUrl: process.env.STUDIO_SUPABASE_LOGIN_URL || '/login',
      },
    },
    i18n: {
      defaultLocale: 'en',
  ...
  async setup(options, nuxt) {
    ...
    nuxt.options.runtimeConfig.studio = {
      auth: {
        sessionSecret: createHash('md5').update([
          ...
          options.auth?.supabase?.url,
          options.auth?.supabase?.key,
        ].join('')).digest('hex'),
    ...
    addServerHandler({
      route: '/__nuxt_studio/auth/supabase',
      handler: runtime('./server/routes/auth/supabase.get'),
    })
    addServerHandler({
